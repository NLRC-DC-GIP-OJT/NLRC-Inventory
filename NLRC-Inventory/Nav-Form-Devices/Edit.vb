...
Public Class Edit
    ...
    Private Sub Edit_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        ' ⚠️ DO NOT fill opstatuscb here – it will be filled in LoadDevice based on DB
    End Sub
    ...
    Public Sub LoadDevice(device As InvDevice)
        ...
        ' Convert specs pointer -> real string, if stored as pointer
        ...
        ' === OPERATIONAL STATUS COMBO (LOAD LIST FROM ENUM IN DB + SELECT CURRENT) ===
        opstatuscb.DataSource = Nothing
        opstatuscb.Items.Clear()
        opstatuscb.DropDownStyle = ComboBoxStyle.DropDownList

        ' 🔴 use ENUM-based function here
        Dim statusList As List(Of String) = mdl.GetStatusEnumValues()

        If statusList IsNot Nothing AndAlso statusList.Count > 0 Then
            opstatuscb.DataSource = statusList

            Dim dbStatus As String = If(device.Status, "").Trim()
            Dim selected As Boolean = False

            If dbStatus <> "" Then
                For i As Integer = 0 To opstatuscb.Items.Count - 1
                    Dim itemText As String = opstatuscb.Items(i).ToString().Trim()
                    If String.Equals(itemText, dbStatus, StringComparison.OrdinalIgnoreCase) Then
                        opstatuscb.SelectedIndex = i
                        selected = True
                        Exit For
                    End If
                Next
    ...
    Private Sub BuildDynamicFields()
        ...
        For Each prop As DataRow In activeProps
            ...
            ' we are handling Status via opstatuscb, so skip dynamic "status" property rows
    ...
    ' ★★ central resize handler so textboxes resize properly
    ...
    Private Sub savebtn_Click(sender As Object, e As EventArgs) Handles savebtn.Click
        ...
        Dim newDevice As New InvDevice With {
                    ...
                    .Ass_Status = currentDevice.Ass_Status,
        ...
        ' === APPLY COMBOBOX VALUE FOR OPERATIONAL STATUS ===
        If opstatuscb.SelectedIndex >= 0 Then
            newDevice.Status = opstatuscb.SelectedItem.ToString()
        End If
        ...
        If currentDevice.Model <> newDevice.Model Then
            ...
            changes.Add(New DeviceChange With {.FieldName = "Model", .OldValue = currentDevice.Model, .NewValue = newDevice.Model})
        End If
        ...
        If currentDevice.SerialNumber <> newDevice.SerialNumber Then
            ...
            changes.Add(New DeviceChange With {.FieldName = "Serial No.", .OldValue = currentDevice.SerialNumber, .NewValue = newDevice.SerialNumber})
        End If
        ...
        If currentDevice.PropertyNumber <> newDevice.PropertyNumber Then
            ...
            changes.Add(New DeviceChange With {.FieldName = "Property No.", .OldValue = currentDevice.PropertyNumber, .NewValue = newDevice.PropertyNumber})
        End If

        ' NSOC NAME
        If currentDevice.NsocName <> newDevice.NsocName Then
            sb.AppendLine($"NSOC Name: {currentDevice.NsocName}  →  {newDevice.NsocName}")
            changes.Add(New DeviceChange With {.FieldName = "NSOC Name", .OldValue = currentDevice.NsocName, .NewValue = newDevice.NsocName})
        End If
        ...
        If Not Nullable.Equals(currentDevice.PurchaseDate, newDevice.PurchaseDate) Then
            ...
            changes.Add(New DeviceChange With {.FieldName = "Purchase Date", .OldValue = fmtDate(currentDevice.PurchaseDate), .NewValue = fmtDate(newDevice.PurchaseDate)})
        End If
        ...
        If Not Nullable.Equals(currentDevice.WarrantyExpires, newDevice.WarrantyExpires) Then
            ...
            changes.Add(New DeviceChange With {.FieldName = "Warranty Expires", .OldValue = fmtDate(currentDevice.WarrantyExpires), .NewValue = fmtDate(newDevice.WarrantyExpires)})
        End If

        ' STATUS (OPERATIONAL)
        If currentDevice.Status <> newDevice.Status Then
            sb.AppendLine($"Status: {currentDevice.Status}  →  {newDevice.Status}")
            changes.Add(New DeviceChange With {.FieldName = "Status", .OldValue = currentDevice.Status, .NewValue = newDevice.Status})
        End If
        ...
        If oldCostStr <> newCostStr Then
            ...
            changes.Add(New DeviceChange With {.FieldName = "Cost", .OldValue = oldCostStr, .NewValue = newCostStr})
        End If

        ' NOTES
        If currentDevice.Notes <> newDevice.Notes Then
            sb.AppendLine("Notes: (changed)")
            changes.Add(New DeviceChange With {.FieldName = "Notes", .OldValue = currentDevice.Notes, .NewValue = newDevice.Notes})
        End If

        ' SPECS
        If currentDevice.Specs <> newDevice.Specs Then
            sb.AppendLine("Specs: (changed)")
            changes.Add(New DeviceChange With {.FieldName = "Specs", .OldValue = currentDevice.Specs, .NewValue = newDevice.Specs})
        End If
        ...
        If currentDevice.BrandPointer <> newDevice.BrandPointer Then
            ...
            changes.Add(New DeviceChange With {.FieldName = "Brand", .OldValue = oldBrandName, .NewValue = newBrandName})
        End If
        ...
        currentDevice.Ass_Status = newDevice.Ass_Status
    ...
    Private Sub LoadHistory(devicePointer As Integer)
        ...
        For Each row As DataRow In dt.Rows
            ...
            If remarks.ToLower().Contains("specs") Then
                ...
                BuildSpecsDiffDisplay(oldRaw, newDisp, oldDisp, newDisp)
